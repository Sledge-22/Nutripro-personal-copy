...
function StudentModuleDetail({ course, completed, onUpdateProgress, progress }) {
  ...
  const videoSource = activeModule?.video_url || activeModule?.videoUrl || activeModule?.video?.url || activeModule?.video?.link || "";
  const pdfSource = activeModule?.pdf_url || activeModule?.pdfUrl || "";

  return <><button className="back-button" onClick={() => goTo(ROUTES.student.courses)}>‹ Back to courses</button><div className="detail-hero"><div><span className="eyebrow">OWNED COURSE</span><h2>{course.title}</h2><p>{course.description}</p></div><div className="hero-progress"><strong>{progress}%</strong><span>Course progress</span><Progress value={progress} /></div></div><div className="course-detail-layout"><aside className="module-list"><div className="module-title"><span className="eyebrow">COURSE MODULES</span><h3>{course.modules.length} modules</h3></div>{course.modules.map((module, index) => <div className="module" key={module.id}><button className={activeModule?.id === module.id ? "active" : ""} onClick={() => { setActiveModuleId(module.id); setViewError(""); }}><span className={`lesson-icon ${completed[`module-${module.id}`] ? "done" : ""}`}>{completed[`module-${module.id}`] ? <Icon name="check" size={14} /> : <span>{index + 1}</span>}</span><span><strong>{module.title}</strong><small>{module.description}</small></span><Icon name="chevron" size={16} /></button></div>)}</aside><section className="lesson-content"><div className="video-stage"><div className="play-large"><Icon name="courses" size={28} /></div><span>Module assets</span></div><span className="eyebrow">CURRENT MODULE</span><h2>{activeModule?.title || "Select a module"}</h2><p>{activeModule?.description || "This module description will appear here for students."}</p>{viewError && <small className="field-note danger-text">{viewError}</small>}<div className="module-assets"><div className="lesson-meta"><span className="subtle-badge">PDF</span><span>{activeModule?.pdfLabel || "No PDF selected"}</span></div>{pdfSource ? <div className="row-actions"><a href={pdfSource} target="_blank" rel="noreferrer" onClick={() => markSeen(`pdf-${activeModule.id}`)}>Open PDF</a></div> : activeModule?.pdfLabel && activeModule.pdfLabel !== "No PDF selected" ? <small className="field-note danger-text">File name exists, but file URL is missing.</small> : <small className="field-note">No PDF uploaded yet.</small>}<div className="lesson-meta"><span className="subtle-badge">Video</span><span>{activeModule?.video.uploadLabel || videoSource || "No video selected"}</span></div>{videoSource ? <div className="video-player-shell"><video controls width="100%" src={videoSource} onPlay={() => markSeen(`video-${activeModule.id}`)} onError={() => { console.error("Video playback failed for module:", activeModule?.id, videoSource); setViewError("The uploaded video could not be viewed."); }} /></div> : activeModule?.video?.uploadLabel && activeModule.video.uploadLabel !== "No video selected" ? <small className="field-note danger-text">File name exists, but file URL is missing.</small> : <small className="field-note">No video uploaded yet.</small>}{activeModule?.video.link && <div className="lesson-meta"><span className="subtle-badge">Video link</span><a href={activeModule.video.link} target="_blank" rel="noreferrer">{activeModule.video.link}</a></div>}</div><div className="progress-steps"><span className={pdfSeen ? "subtle-badge" : "count-badge"}>{pdfSeen ? "PDF viewed" : "PDF pending"}</span><span className={videoSeen ? "subtle-badge" : "count-badge"}>{videoSeen ? "Video viewed" : "Video pending"}</span></div><button className={moduleDone ? "complete-btn done" : "complete-btn"} onClick={toggleModule} disabled={!activeModule || !canComplete}><Icon name="check" />{moduleDone ? "Module marked complete" : "Mark module complete"}</button></section></div></>;
}
